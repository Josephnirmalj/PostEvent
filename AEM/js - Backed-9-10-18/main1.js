$(document).ready(function(e) {
			...
			//document.cookie = "pageLoadView="+1+";path=/";
			...
			if(getCookieOnload == ""){
				...
				getCookieOnload++;
				document.cookie = "pageLoadView="+getCookieOnload+";path=/";
			}
			//static designs
			$("body").addClass("overflow-active");
			$(".subnav-view .expand-mobile-menu span").text("Replays");
			//dynamic designs
		...
		var visiorEmail="";
		var eloquaFormName="19Q2-APJ-ANZ-DTF-PostEvent-Investigation";
		var SiteID = "2502";
		var assetName ="";
		...
		function conprepopulate() {
            ...
            //var visiorEmail1 = GetElqContentPersonalizationValue('V_ElqEmailAddress');
		...
		function ajaxFormSubmit(artCatogrey,artTitle){
			var videoCookie=getCookie("totalVideoView");
			var articleCookie=getCookie("totalArticleView");
			...
			var totalArticlesClicked = parseInt(videoCookie)+parseInt(articleCookie);
			//alert(visiorEmail);
			...
			var data = "emailAddress="+visiorEmail+"&assetName="+artTitle+"&assetCatogrey="+artCatogrey+"&viewedVideos="+videoCookie+"&formSubmits="+articleCookie+"&webVisit="+pageViewCount;
			$.ajax({
				url:url,
				type: "POST",
				data:data,
			...
			if(articleCookie == ""){
				...
				if(errmsg == ""){
					var url = "//s2502.t.eloqua.com/e/f2?elqFormName="+eloquaFormName+"&elqSiteID="+SiteID+"&";
					var data="emailAddress="+email+"&optin="+optin+"&viewedVideos="+videoCookie+"&formSubmits="+articleCookie+"&webVisit="+pageViewCount;;
					$.ajax({
				url:url,
				type: "POST",
				data:data,
				...
				error: function (xhr, type, exception){
					...
					document.cookie = "optinformSubmitted="+"yes"+";path=/";
				}
				else{
					return false;
				}
	...
	$(".video-doc-content").on("click",function(){
			var countV="";
			var countA="";
			var getVideoArticleID = $(this).attr("id");
			document.cookie = getVideoArticleID + "="+1+";path=/";
			document.cookie = "asset_name="+getVideoArticleID+";path=/";
			...
			document.cookie = "totalVideoView="+countV+";path=/";	
		...
		myPlayer.catalog.getVideo(dataVideo[arraySelectionID].videoPlayerURL, function(error, video) {
		...
		$(".VideoSpeakerDesc1").text(dataVideo[arraySelectionID].VideoSpeakerDesc1);
		$(".VideoSpeakerName2").text(dataVideo[arraySelectionID].VideoSpeakerName2);
		$(".VideoSpeakerDesc2").text(dataVideo[arraySelectionID].VideoSpeakerDesc2);
		$(".FBShareURL").attr("href",dataVideo[arraySelectionID].FBShareURL);
		$(".TwitterShareURL").attr("href",dataVideo[arraySelectionID].FBShareURL);
	...
	$(".pdf-doc-content").on("click",function(){
		var countV="";
		var countA="";
		var getPDFArticleID = $(this).attr("id");
		document.cookie = getPDFArticleID + "="+1+";path=/";
		document.cookie = "asset_name="+getPDFArticleID+";path=/";
		var getCommonLength = $(".pdf-doc-content").length;
			...
			document.cookie = "totalArticleView="+countA+";path=/";
		...
		$(".ArtLinkedinUrl").attr("href",dataArticle[arraySelectionID].ArtTwitterUrl);
		...
		$(".artSpeakerDecs1").text(dataArticle[arraySelectionID].artSpeakerDecs1);
		$(".artSpeakerName2").text(dataArticle[arraySelectionID].artSpeakerName2);
		$(".artSpeakerDecs2").text(dataArticle[arraySelectionID].artSpeakerDecs2);
	...
	$(".close-btn").on("click",function(){
		...
		$("#btn-submit").on("click",function(){
			...
			if(errMsg == ""){
				...
				var data ="&firstName="+fname+"&lastName="+lname+"&emailAddress="+email+"&busPhone="+busPhone+"&company="+company+"&jobFunction1="+jobTitle+"&country="+country+"&clickSrcTitle="+clickSrcTitle+"&clickSrcCat="+clickSrcCat+"&comments="+comments;
				$.ajax({
				url:"//s2502.t.eloqua.com/e/f2?elqFormName=19Q2-APJ-ANZ-DTF-PostEvent-Investigation-Gate&elqSiteID=2502",
				type: "POST",
				data:data,
				crossDomain: true,
	...
	/*var iframe = $("iframe").contents();
        ...
        iframe.find("#contactUsPdf").click(function(){
               alert("test");
        });*/
});